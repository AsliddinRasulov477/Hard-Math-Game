...
class Games: UIViewController {
    ...
    @IBAction func numberActions(_ sender: MDCFloatingButton) {
        ...
        sender.setBorderColor(.systemRed, for: .normal)
        push += 1
        ...
        if push == 2 && secondType.text == "÷" {
            ...
            if !forHelp && multiply[0].center != multiply[1].center {
                for i in numbersCollection.indices {
                    numbersCollection[i].setBorderColor(.white, for: .normal)
    ...
    @objc func panButton(pan: UIPanGestureRecognizer) {
        ...
        button?.setBorderColor(.systemRed, for: .normal)
        ...
        if pan.state == .ended || pan.state == .failed || pan.state == .cancelled {
            ...
            for i in numbersCollection.indices {
                if sqrt(pow(location.x - numbersCollection[i].center.x, 2) + pow(location.y - numbersCollection[i].center.y, 2)) < button!.frame.width {
                    numbersCollection[i].setBorderColor(.systemRed, for: .normal)
    ...
    @IBAction func refreshBurrons(_ sender: UIButton) {
        ...
        let vc = storyboard?.instantiateViewController(identifier: "Games") as! Games
        vc.time = time
        vc.level = level
        vc.result = result
        vc.numbers = numbers
        ...
        if oneHelp {
            ...
            for i in numbersCollection.indices {
                if numbersCollection[i].currentTitle == doubleToString(result: abs(helpNumbers[0])) {
                    numbersCollection[i].setBorderColor(.green, for: .normal)
                    break
                }
            }
            for i in numbersCollection.indices {
        ...
        if numbersCollection.count == 1 && numbersCollection[0].currentTitle == goalValue.text {
            ...
            if level == 8 {
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                    let vc = self.storyboard?.instantiateViewController(identifier: "Levels") as! Levels
                ...
                let nextButton = MDCFloatingButton(frame: CGRect(x: (view.frame.width - 0.15 * view.frame.height) / 2, y: 0.65 * view.frame.height, width: 0.15 * view.frame.height, height: 0.15 * view.frame.height))
                nextButton.backgroundColor = .white
                nextButton.setTitleColor(.systemRed, for: .normal)
                nextButton.setBorderWidth(4, for: .normal)
                nextButton.setBorderColor(.systemRed, for: .normal)
    ...
    @IBAction func back(_ sender: UIButton) {
        ...
        let vc = storyboard?.instantiateViewController(identifier: "Levels") as! Levels
